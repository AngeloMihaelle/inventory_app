...
class CSVImportResult {
  ...
  CSVImportResult({required this.success, this.items = const [], this.error});
}
...
class CSVService {
  ...
  Future<bool> exportToCSV(List<Item> items) async {
    try {
      ...
      // Create file path
      final fileName =
          'inventario_${DateTime.now().millisecondsSinceEpoch}.csv';
  ...
  Future<CSVImportResult> importFromCSV() async {
    try {
      ...
      return CSVImportResult(success: true, items: items);
    } catch (e) {
      return CSVImportResult(
        success: false,
